export namespace Shapes {
  ...
  // Shapes
  // These are wrappers for data validastion logic that can be mapped down to actual TS types.
  ...
  interface ObjectShape<T extends ObjectProperties = ObjectProperties> extends TypeShape {
  ...
  interface ArrayShape<T extends TypeShape> extends TypeShape {
    typename: `array<${this["members"]["typename"]}>`;
    members: T;
  ...
  export function optional<T extends TypeShape>(shape: T): OptionalShape<T> {
    return {
      ...
      checkFn: (input: unknown): input is T["primitive"] | null => isNull(input) || shape["checkFn"](input),
      inner: { ...shape, optional: true },
    } as never;
  }
  ...
  export function object<T extends ObjectProperties>(properties: T): ObjectShape<T>;
  export function object<T extends ObjectProperties>(name: string, properties: T): ObjectShape<T>;
  export function object<T extends ObjectProperties>(nameOrProps: string | T, maybeProperties?: T): ObjectShape<T> {
      ...
      checkFn: (input: unknown): input is Type<ObjectShape<T>> => {
        ...
        for (const inputProperty in input) {
          if (!(inputProperty in properties)) {
            console.log(`Checking ${typename} failed due to unknown property ${inputProperty} in input data`);
            return false;
          }
        }
        ...
        for (const property in properties) {
          ...
          if (!(property in input)) {
            console.log(`Checking ${typename} failed due to missing property "${property}"`);
            return false;
          }

          // Property type check
          const field = (input as { [property: string | number]: unknown })[property];
